...
class m3u8_dl(object):
    def __init__(self, url, out_path, proxy, auth, not_verify_ssl, custom_key, debug):
        ...
        self.ts_list = [
            urljoin(self.url, n.strip())
            for n in self.m3u8_content.split("\n")
    ...
    def readkey(self):
        tag_list = [
            n.strip() for n in self.m3u8_content.split("\n") if n and n.startswith("#")
        ]
        for s in tag_list:
            if str.upper(s).startswith("#EXT-X-KEY"):
                ...
                if r.status_code == 200:
                    return r.content
                logger.fatal(
                    f"Can`t download key url: {uri}, maybe you should use proxy"
                )
                sys.exit(-1)
    ...
    def try_merge(self):
        ...
        while self.next_merged_id < self.length:
            ...
            pp.update(
                "total merged ", self.next_merged_id, self.length, "", userDefineVisual2
            )
            pp.update(
                "block pending",
                self.next_merged_id + len(self.ready_to_merged),
                self.length,
                "",
                userDefineVisual2,
            )
            ...
            try:
                if self.next_merged_id in self.ready_to_merged:
                    ...
                    p = os.path.join(
                        self.tempdir, self.tempname, str(self.next_merged_id)
                    )
                    ...
                    logger.debug(
                        f"unmerged {self.ready_to_merged} active_thread:{threading.active_count()}"
                    )
            except Exception as e:
                logger.exception(e)
                try:
...
def createParse():
    ...
    parser.add_argument(
        "-p", "--proxy", type=str, help="for example: socks5h://127.0.0.1:5992"
    )
    ...
    parser.add_argument(
        "-d", "--debug", help="debug info", default=False, action="store_true"
    )
    parser.add_argument(
        "-w", "--overwrite", help="overwrite existed file", action="store_true"
    )
    ...
    parser.add_argument(
        "-k",
        "--ignore_certificate_verfication",
