...
class D():
        ...
        while True: 
            try:
                ...
                if resp.status_code>=200:
                    ...
                    logger.debug("total size:" + str( total_size ))
                    with open(destFile, "ab") as f:
                        block_size = 1024
                        wrote = localSize
                        for data in tqdm(resp.iter_content(block_size), initial=wrote / block_size, total=webSize / block_size,unit='kb', unit_scale=True):
                            if data:
    ...
    def getWebFileSize(self, url):
        ...
        if 'Content-Length' not in  rr.headers:
            raise Exception(f"no size in head {url}")
        if not (300>rr.status_code>=200):
